...
export default function Logistics() {
  ...
  return (
    <div className={`pt-28 pb-28 bg-gray-100 w-auto h-auto ${lang === "AR" ? "text-right" : "text-left"}`}>
      {/* Hero Section */}
      ...
      <div className="bg-gray-100 py-16 px-4 md:px-12 lg:px-24 flex flex-col items-center">
        <div className="bg-white rounded-2xl shadow-md p-6 md:p-10 w-full md:w-4/5">
          <h2 className="text-3xl md:text-4xl font-serif mb-4 text-gray-800">
            {lang === "EN" ? "Market Presence & Export Countries" : "تواجدنا في الأسواق العالمية"}
          </h2>
          <p className="text-gray-600 text-base md:text-lg leading-relaxed">
            {lang === "EN"
              ? "For over a decade, HASBANI products have traveled to markets around the world, supported by strong partnerships and growing international demand, and trusted in some regions even before carrying HASBANI name."
              : <p>
  على مدى أكثر من عقد من الزمن، وصلت منتجاتنا إلى أسواق كثيرة حول العالم، مدعوما بشراكات قوية
  وطلب دولي متزايد، واكتسبت الثقة بسبب جودتها وطعمها المتميّز حتى قبل أن تحمل اسم{' '}
...
<section className="bg-[#414A38] text-[#DDE3C1] py-20 px-6 md:px-16" dir={lang === "AR" ? "rtl" : "ltr"}>
  <div className="text-center mb-12">
    <h2 className={`text-3xl md:text-4xl font-serif font-semibold mb-4 ${lang === "AR" ? "text-right" : "text-left"}`}>
      {lang === "EN" ? "Work With Us" : "تشارك معنا"}
    </h2>
    <p className={`text-sm md:text-base leading-relaxed ${lang === "AR" ? "text-right" : "text-left"}`}>
      {lang === "EN"
...
{/* Card 3 */}
<div className="bg-white text-[#414A38] rounded-2xl p-8 shadow-md" dir={lang === "AR" ? "rtl" : "ltr"}>
  <div className={`flex items-start gap-4 ${lang === "AR" ? "" : ""}`}>
      ...
      <ul className="list-disc list-inside text-sm leading-relaxed">
        {lang === "EN" ? (
          <>
            ...
            <li>Land transport – for regional and neighboring markets</li>
          </>
        ) : (
          <>
            <li>الشحن البحري – مثالي للطلبات الكبيرة</li>
            <li>الشحن الجوي – للطرود العاجلة أو الصغيرة</li>
            <li>النقل البري – للأسواق المحليّة، الإقليمية والدول المجاورة</li>
          </>
        )}
      </ul>
    </div>
...
>
      ...
      <ul className="list-disc list-inside text-sm leading-relaxed">
        {lang === "EN" ? (
          ...
          <>
            ...
            <li>شروط الشحن: FOB بشكل افتراضي، مع إمكانية CIF أو DDP عند الطلب</li>
            <li>مدة التسليم: 10، 30 أو 60 يومًا بحسب الوجهة واللوجستيات</li>
            <li>خيارات التغليف: متاحة بوحدات جاهزة للبيع بالتجزئة أو بطلبات بالجملة</li>
            <li>جاهزون للتصدير مع جميع المستندات المطلوبة، ودعم كامل في التخليص الجمركي والشهادات</li>
          </>
        )}
      </ul>
    </div>
  ...
  {/* Footer Text */}
  <div className={`mt-12 text-sm md:text-base ${lang === "AR" ? "text-right" : "text-left"}`}>
    <p>{lang === "EN" ? "Export-ready with all required documentation" :` تواصل معنا
`}</p>
    <p>{lang === "EN" ? "Support with customs clearance & certifications" : `لمزيد من المعلومات حول التعاون، التوزيع، أو فرص الشراكة مع HASBANI ، يُرجى التواصل
معنا عبر القنوات الرسمية .
`}</p>
  </div>
</section>
